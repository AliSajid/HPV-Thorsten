---
title: "HPV +/- with/out radiation at 15 min"
subtitle: "Nanik & Thorsten"
...
params:
  chip_type: STK
  signal_file: "kinome_data/E5-_ExporttComplete_Median_SigmBg_260907091348.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/E5-_ExporttComplete_Signal_Saturation_260907091349.txt" # Change the file name to the one you have
  threshold: 2 # Change the threshold if necessary
  prefix: hpv_pos_neg_26.2
  pairwise: FALSE
  figure_output: NULL
---
...
qc_data <- krsa_qc_steps(raw_data) |>
    mutate(
           Group = case_when(
                str_detect(SampleName, "HPV+") && str_detect(SampleName, "0 Gy") ~ "PosCTL"
                str_detect(SampleName, "HPV+") && str_detect(SampleName, "6 Gy") ~ "PosTRT"
                str_detect(SampleName, "HPV-") && str_detect(SampleName, "0 Gy") ~ "NegCTL"
                str_detect(SampleName, "HPV-") && str_detect(SampleName, "6 Gy") ~ "NegTRT"
           ),
        SampleName = paste0(Group, "_", str_extract(Barcode, "\\d{3}$"))
    )

groups <- unique(qc_data[["Group"]])

group_labels  <- c(
                   PosCTL = "HPV+ Untreated",
                   PosTRT = "HPV+ Treated 6 Gy / 15 m",
                   NegCTL = "HPV- Untreated",
                   NegTRT = "HPV- Treated 6 Gy / 15 m",
)
...
- `r group_labels[[groups[[2]]]]` vs `r group_labels[[groups[[1]]]]`
- `r group_labels[[groups[[4]]]]` vs `r group_labels[[groups[[3]]]]`
- `r group_labels[[groups[[1]]]]` vs `r group_labels[[groups[[3]]]]`
- `r group_labels[[groups[[2]]]]` vs `r group_labels[[groups[[4]]]]`
