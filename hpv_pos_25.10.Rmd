---
title: "HPV Positive - 5,15,30 Minutes"
...
params:
  chip_type: STK
  signal_file: "kinome_data/E1_ExporttComplete_Median_SigmBg_260914134901.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/E1_ExporttComplete_Signal_Saturation_260914134902.txt" # Change the file name to the one you have
  threshold: 2 # Change the threshold if necessary
  prefix: hpv_pos_25.10
  pairwise: FALSE
  figure_output: NULL
---
...
qc_data <- krsa_qc_steps(raw_data) |>
    mutate(
        Group = case_when(
            str_detect(SampleName, "0Gy") ~ "PosCTL",
            str_detect(SampleName, "30m") ~ "Pos30m",
            str_detect(SampleName, "15m") ~ "Pos15m",
            str_detect(SampleName, "5m") ~ "Pos5m",
        ),
...
group_labels <- c(
    PosCTL = "HPV+ Samples / 0 Gy",
    Pos30m = "HPV+ Samples / 6 Gy / 30 m",
    Pos15m = "HPV+ Samples / 6 Gy / 15 m",
    Pos5m = "HPV+ Samples / 6 Gy / 5 m"
)
