---
title: "HPV Negative - 5,15,30 Minutes"
...
params:
  chip_type: STK
  signal_file: "kinome_data/E3-_ExporttComplete_Median_SigmBg_260907093012.txt" # Change the file name to the one you have
  saturation_file: "kinome_data/E3-_ExporttComplete_Signal_Saturation_260907093013.txt" # Change the file name to the one you have
  threshold: 2 # Change the threshold if necessary
  prefix: hpv_neg_25.12
  pairwise: FALSE
  figure_output: NULL
---
...
qc_data <- krsa_qc_steps(raw_data) |>
    mutate(
        Group = case_when(
            str_detect(SampleName, "0Gy") ~ "NegCTL",
            str_detect(SampleName, "30m") ~ "Neg30m",
            str_detect(SampleName, "15m") ~ "Neg15m",
            str_detect(SampleName, "5m") ~ "Neg5m",
        ),
...
group_labels <- c(
    NegCTL = "HPV- Samples / 0 Gy",
    Neg30m = "HPV- Samples / 6 Gy / 30 m",
    Neg15m = "HPV- Samples / 6 Gy / 15 m",
    Neg5m = "HPV- Samples / 6 Gy / 5 m"
)
...
- `r group_labels[[groups[[3]]]]` vs `r group_labels[[groups[[1]]]]`
- `r group_labels[[groups[[4]]]]` vs `r group_labels[[groups[[1]]]]`
...
comparisons <- list(
    ...
    COMP2 = c(groups[[3L]], groups[[1L]]),
    COMP3 = c(groups[[4L]], groups[[1L]])
)
```
